...
class Coco(data.Dataset):
    ...
    def __init__(self, export=False, transform=None, task='train', **config):
        ...
        self.init_var()
        if export:
            self.had_npz=os.listdir(r"C:\Users\Administrator\PycharmProjects\pythonProject\logs/superpoint_my_data/predictions/"+task)
        else:
            self.had_npz=[]
        pass
    ...
    def __getitem__(self, index):
        ...
        if sample['name']+'.npz' in self.had_npz:
            print('sample:::::',sample)
            return input
